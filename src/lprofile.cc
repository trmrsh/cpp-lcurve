...
!!emph{lprofile} computes the line profile from a Roche distorted star, allowing for irradiation of one face.
The radiation from the star is modelled as a slab of constant optical depth with a source function
that changes exponentially with depth. This allows one to have a continuous transition from 
optically thin to thick and have limb darkening or brightening. The computation is carried out for the secondary star
only.
...
int main(int argc, char* argv[]){
  ...
  try{
    ...
    Lcurve::set_star_grid(model, Roche::SECONDARY, false, star2);
    ...
    const double GDCBOL2 = model.gdark_bolom2 ? model.gravity_dark2 : model.gravity_dark2 / Subs::dlpdlt(model.wavelength, model.t2);

    // Calculate element surface brightnesses, applying gravity darkening, i.e. assuming constant
    // equivalent width
