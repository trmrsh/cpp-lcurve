...
/**
 * set_star_grid sets up the elements needed to define the primary(secondary) star in a co-rotating binary system. For each element 
 * set_star_grid computes the position within the binary, the direction perpendicular to the element, the area and gravity of the element, 
 * (optionally) whether it is eclipsed by the secondary(primary) star, and if so, what are the ingress
 * and egress phases of the eclipse. This is optional to allow a saving of time in cases where you are nor interested
 * in whether the elements are eclipsed.
 *
 ...
 * the 'North Pole' of the star taken as pointing towards its companion. Each latitude strip has a constant latitude
 * decrement and is split into a number of equal longitude elements.  Measuring from the North pole, strip 'n'
 * run from Pi*n/nlat to Pi*(n+1)/nlat, with n running from 0 to nlat-1. At each latitude a number of longitude strips
 * equal to max(8,2*nlat*sin(theta)) will be selected where 'theta' is the angle from the North pole at the centre of
 * the latitude strip. This gives roughly square elements. By defining the grid in terms of an integer we ensure that during
 * optimsation, the number of grid elements is fixed, although their sizes and exact positions etc may vary as parameters such 
 * as mass ratio are varied. 
 * 
...
void Lcurve::set_star_grid(const Model& mdl, Roche::STAR which_star, bool fine, Subs::Buffer1D<Lcurve::Point>& star){

  double r1, r2;
  mdl.get_r1r2(r1, r2);

  bool eclipse = which_star == Roche::PRIMARY ? mdl.eclipse1 : mdl.eclipse2;
  int nlat     = which_star == Roche::PRIMARY ? (fine ? mdl.nlat1f : mdl.nlat1c) : (fine ? mdl.nlat2f : mdl.nlat2c);
  ...
  // Gravitational lensing accounted for here by shrinking the lensing star by a fixed amount when it is
  // not the one having its grid computed. This enables a proper computation of the eclipse of the star
  // having its grif computed by its companion. The proper correction to the radius is:
  //   r --> r - 4GMd/(c^2 r)
  // where d is the distance from the limb of the other star to the point in question. 
  // Here it is assumed that d = a - R/2, the orbital separation minus half the radius 
  // of the star being eclipse, which is approximate but should not be too bad.
  ...
  if(infill){
	...
	if(xy.x*xy.x+xy.y*xy.y < r2*r2){
      ...
      thehi = std::max(Constants::PI/2.-Subs::deg2rad(latlo), std::min(Constants::PI/2. + rangle, acos(xy.y*sini/r2) + Subs::deg2rad(mdl.lfudge)));
    }
    ...
    if(ratio >= 1.){
      ...
      thelo = std::min(Constants::PI/2.-Subs::deg2rad(lathi), std::max(0., Constants::PI/2. - acos(ratio) + rangle - Subs::deg2rad(mdl.lfudge)));
    }
  }
  ...
  // The grid starts at the North pole and ends at the South, proceeding in a series of equi-latitudinal rings.
  // The polar axis is parallel to the x-axis which points from one star to the other. The North pole is defined
  // to be the point closest to the other star (or the genuine North Pole if npole is true). The angle theta is 
  // measured away from the North pole, the angle phi is measured from the Y axis towards the Z axis.
  ...
  if(infill){
    add_faces(star, nface, 0., thelo, dtheta, 0, 0, mdl.npole, which_star, mdl.q, mdl.iangle, 
	      r1, r2, rref1, rref2, mdl.roche1, mdl.roche2, mdl.spin1, mdl.spin2, eclipse, gref, pref1, pref2, ffac1, ffac2, mdl.delta_phase);
    add_faces(star, nface, thelo, thehi, dtheta, nlatfill, nlngfill, mdl.npole, which_star, mdl.q, mdl.iangle, 
	      r1, r2, rref1, rref2, mdl.roche1, mdl.roche2, mdl.spin1, mdl.spin2, eclipse, gref, pref1, pref2, ffac1, ffac2, mdl.delta_phase);
...
void Lcurve::add_faces(Subs::Buffer1D<Lcurve::Point>& star, int& nface, double tlo, double thi, double dtheta, int nlatfill, int nlngfill, 
    ...
    if(infill){
	// If infill is True we loop through the latitudes once on the side facing the other star and once on the other side
	// so that infilling only occurs on one side. The infilled part will be included first
...
#pragma omp parallel for schedule(dynamic)
	...
	for(int np=0; np<nphi; np++){
	    ...
	    // Direction is now defined, so calculate radius and thus the position according to
	    // whether we are accounting for Roche geometry or not.
	    ...
	    // Eclipse computation. We calculate whether a point is eclipsed, and, if it is, its ingress and
	    // egress phases. Account for spherical or Roche geometry of other star. Since the stars are convex
	    // this calculation only accounts for eclipse by the OTHER star. 
