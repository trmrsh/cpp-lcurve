...
/**
 * set_bright_spot_grid sets up the elements needed to define the bright spot at the edge of the disc. This is modelled
 * as two straight lines of elements coincident in position, but with one oriented parallel to the plane of the disc and 
 * the other tilted by an arbitrary angle. This is to allow the elements to make a hump while allowing flexibility in the step
 * heights of the bright-spot ingress and egress features. The brightness of the spot along the lines is modelled as
 * a function of distance x from the start of each line as (x/l)**p1*exp(-(x/l)**p2) where l is a scale length and p1 and p2 are power 
 * law exponents. The maximum of this occurs at x = l*(p1/p2)**(1/p2), and is set to be the intersection of the gas stream with 
